...
# random points in each zone
zones_cases <- schools_zones %>%
  filter(!is.na(zona)) %>%
  select(unit, zona) %>%
  left_join(usac_units) %>%
  group_by(zona) %>%
  summarize(
    n = sum(n)
  ) %>%
  left_join(usac_zones) %>%
  sf::st_as_sf() %>%
  print()

schools_points <- zones_cases %>%
  sf::st_sample(size = .$n, by_polygon = TRUE)
